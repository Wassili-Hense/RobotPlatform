...
static bool SendCommandNow(const uint8_t* data, uint8_t len, bool requiresLcdReady, bool isLcd)
{
    if (data == nullptr || len == 0U || len > 32U)
    {
        return false;
    }
    if (requiresLcdReady && !s_lcdSendAllowed)
    {
        return false;
    }
...
bool hmi_cmd_set_backlight_timeout(uint32_t timeout_ms)
{
    ...
    return SendCommandNow(data, sizeof(data), false, false);
}
...
bool hmi_cmd_lcd_clear(uint16_t rgb565_color)
{
    ...
    return SendCommandNow(data, sizeof(data), true, true);
}
...
bool hmi_cmd_lcd_draw_text(uint8_t x, uint8_t y, uint16_t rgb565_color, const char* text)
{
    ...
    data[5U + textLen] = 0U;
    return SendCommandNow(data, (uint8_t)(6U + textLen), true, true);
}
