...
enum pec_state_t : uint8_t { PEC_DISCOVERY,
                             PEC_CONNECTING,
                             PEC_AUTHING,
                             PEC_SECURE_WAIT,
                             PEC_SECURE };
enum pec_event_type_t : uint8_t { PEC_EVT_DISC,
                                  PEC_EVT_CONNECT_RSP,
                                  PEC_EVT_AUTH_RSP,
                                  PEC_EVT_VAR_I,
                                  PEC_EVT_VAR_F,
...
static bool EmitRxEvent(const pec_rx_event_t& ev) {
  return (s_rxEvent != nullptr) && s_rxEvent(&ev);
}
...
static void ClearStreamCache(void) {
...
static bool IsStateMsg(uint8_t type) {
  return (type == MSG_STATE_I_VAR) || (type == MSG_STATE_F_VAR);
}
static bool IsEventMsg(uint8_t type) {
  return (type == MSG_EVENT_I_VAR) || (type == MSG_EVENT_F_VAR);
}
static bool IsSecureMsg(uint8_t type) {
  return (type == MSG_STREAM_I_VAR) || (type == MSG_STREAM_F_VAR) || (type == MSG_STATE_I_VAR) || (type == MSG_STATE_F_VAR) || (type == MSG_EVENT_I_VAR) || (type == MSG_EVENT_F_VAR) || (type == MSG_ACK) || (type == MSG_NACK) || (type == MSG_GET_VAR);
}
...
static bool SendAckTo(const uint8_t mac[6], uint16_t ackSeq, uint32_t varId) {
  ...
  else EmitError(PEC_HW_ERR_SEND, MSG_ACK);
  return ok;
}
...
static bool SendNackTo(const uint8_t mac[6], uint16_t ackSeq, uint32_t varId, uint8_t reason) {
  ...
  if (ok) s_lastTxMs = millis();
  else EmitError(PEC_HW_ERR_SEND, MSG_NACK);
  return ok;
}
...
static bool SendVarIRaw(uint32_t varId, int32_t value, uint16_t ttlMs) {
  ...
  if (ok) s_lastTxMs = millis();
  else EmitError(PEC_HW_ERR_SEND, MSG_STREAM_I_VAR);
  return ok;
}
...
static bool SendVarFRaw(uint32_t varId, float value, uint16_t ttlMs) {
  ...
  if (ok) s_lastTxMs = millis();
  else EmitError(PEC_HW_ERR_SEND, MSG_STREAM_F_VAR);
  return ok;
}
...
static bool SendVarI(uint8_t msgType, uint32_t varId, int32_t value, uint16_t ttlMs) {
  ...
  if (ok) {
    s_lastTxMs = millis();
    if (msgType == MSG_STREAM_I_VAR) CacheStreamI(varId, value, ttlMs);
...
static bool SendVarF(uint8_t msgType, uint32_t varId, float value, uint16_t ttlMs) {
  ...
  if (ok) {
    s_lastTxMs = millis();
    if (msgType == MSG_STREAM_F_VAR) CacheStreamF(varId, value, ttlMs);
  } else EmitError(PEC_HW_ERR_SEND, msgType);
  return ok;
}
...
static void ProcessEvent(const pec_event_t& ev) {
  if ((ev.type == PEC_EVT_DISC) && (s_state == PEC_DISCOVERY)) {
    ...
    if ((ev.sessionId != 0U) || (memcmp(p->devMac, ev.mac, 6U) != 0)) {
      ResetToDiscovery(PEC_LINK_MAC_BAD);
      return;
    }
    ...
    s_stateStartMs = millis();
    s_lastTxMs = 0U;
    (void)EmitLink(PEC_LINK_CONNECTED);
    ...
    if ((ev.sessionId == s_sessionId) && (memcmp(p->devProof, s_devProof, 16U) == 0)) {
      (void)EmitLink(PEC_LINK_AUTH_OK);
...
static void Task(void*) {
  ...
  for (;;) {
    ...
    if ((s_state == PEC_DISCOVERY) && ((now - s_lastTxMs) >= PEC_DISCOVERY_MS)) {
      ...
      if ((now - s_stateStartMs) > PEC_AUTH_TIMEOUT_MS) ResetToDiscovery(PEC_LINK_CONN_TO);
      else if ((now - s_lastTxMs) >= PEC_RETRY_MS) {
        s_lastTxMs = now;
        (void)SendConnect();
      }
    } else if (s_state == PEC_AUTHING) {
      if ((now - s_stateStartMs) > PEC_AUTH_TIMEOUT_MS) ResetToDiscovery(PEC_LINK_AUTH_TO);
      else if ((now - s_lastTxMs) >= PEC_RETRY_MS) {
        s_lastTxMs = now;
        (void)SendAuth();
      }
      ...
      if ((s_secureInstallAtMs != 0U) && ((int32_t)(now - s_secureInstallAtMs) >= 0)) {
        if (!InstallEncryptedPeer()) ResetToDiscovery(PEC_LINK_SEC_BAD);
      }
    } else if (s_state == PEC_SECURE) {
      if ((now - s_lastRxMs) >= PEC_LINK_TIMEOUT_MS) ResetToDiscovery(PEC_LINK_LOST);
      else (void)SendStreamHeartbeat(now);
    }
...
bool pec_send_get_var(uint32_t varId) {
  ...
  if (ok) s_lastTxMs = millis();
  else EmitError(PEC_HW_ERR_SEND, MSG_GET_VAR);
  return ok;
}
...
bool pec_send_state_i(uint32_t varId, int32_t value, uint16_t ttlMs) {
  return SendVarI(MSG_STATE_I_VAR, varId, value, ttlMs);
}
bool pec_send_state_f(uint32_t varId, float value, uint16_t ttlMs) {
  return SendVarF(MSG_STATE_F_VAR, varId, value, ttlMs);
}
