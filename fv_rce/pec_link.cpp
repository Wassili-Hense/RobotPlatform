...
static constexpr uint32_t PEC_PING_MS = 500U;
static constexpr uint32_t PEC_LINK_TIMEOUT_MS = 2500U;
...
static uint32_t s_lastPongMs = 0U;
...
static bool Line(const char* text) {
    return (s_line != nullptr) && (text != nullptr) && s_line(text);
}
...
static void ResetToDiscovery(const char* reason) {
    if (s_peerValid && MacIsSet(s_peerMac) && esp_now_is_peer_exist(s_peerMac)) {
        (void)esp_now_del_peer(s_peerMac);
    }
...
static bool SecureEventOk(const pec_event_t& ev) {
    return (s_state == PEC_SECURE) && s_peerValid && (ev.sessionId == s_sessionId) && (memcmp(ev.mac, s_peerMac, 6U) == 0);
}
...
static void QueueEvent(uint8_t eventType, uint8_t msgType, uint32_t sessionId, const uint8_t* mac, int8_t rssi, uint16_t seq, const void* payload, size_t payloadLen) {
    ...
    for (size_t i = 0; i < PEC_EVENT_CAP; ++i) {
        if (!s_events[i].used) {
            ...
            if ((payload != nullptr) && (payloadLen <= sizeof(s_events[i].payload))) {
                memcpy((void*)s_events[i].payload, payload, payloadLen);
            }
            portEXIT_CRITICAL(&s_mux);
            return;
        }
...
static void OnRecv(const esp_now_recv_info_t* info, const uint8_t* data, int len) {
    ...
    else if (type == MSG_PONG) QueueEvent(PEC_EVT_PONG, type, hdr->sessionId, info->src_addr, rssi, hdr->seq, payload, sizeof(pen_pong_payload_t));
...
static bool SendAckTo(const uint8_t mac[6], uint16_t ackSeq, uint32_t varId) {
    ...
    if (!ok) EmitError(PEC_HW_ERR_SEND, MSG_ACK);
    return ok;
}
...
static bool SendNackTo(const uint8_t mac[6], uint16_t ackSeq, uint32_t varId, uint8_t reason) {
    ...
    if (!ok) EmitError(PEC_HW_ERR_SEND, MSG_NACK);
    return ok;
}
...
static bool SendConnect(void) {
    ...
    if (!pen_add_peer(s_peerMac, false, nullptr)) {
        EmitError(PEC_HW_ERR_ESPNOW, MSG_CONNECT_REQ);
        return false;
    }
...
static bool InstallEncryptedPeer(void) {
    if (esp_now_set_pmk(s_pmk) != ESP_OK) {
        EmitError(PEC_HW_ERR_ESPNOW, 100);
        return false;
    }
    if (!pen_add_peer(s_peerMac, true, s_lmk)) {
        EmitError(PEC_HW_ERR_ESPNOW, 101);
        return false;
    }
    s_state = PEC_SECURE;
    s_lastPongMs = millis();
...
static bool SendPing(void) {
    pen_ping_payload_t p = {};
    p.pingSeq = s_pingSeq++;
    const bool ok = pen_send_frame(s_peerMac, MSG_PING, s_sessionId, s_seq++, p);
    if (!ok) EmitError(PEC_HW_ERR_SEND, MSG_PING);
    return ok;
}
...
static bool SendVarI(uint8_t msgType, uint32_t varId, int32_t value, uint16_t ttlMs) {
    ...
    if (!ok) EmitError(PEC_HW_ERR_SEND, msgType);
    return ok;
}
...
static bool ForwardVarIToPc(const pen_var_i_payload_t* p, uint8_t msgType, uint16_t seq) {
    char name[5];
    char line[32];
    pen_var_id_to_text(p->varId, name);
...
static void ProcessEvent(const pec_event_t& ev) {
    ...
    if ((ev.type == PEC_EVT_DISC) && (s_state == PEC_DISCOVERY)) {
        ...
        snprintf(line, sizeof(line), "@DISC %d", (int)ev.rssi);
        (void)Line(line);
        s_state = PEC_CONNECTING;
        s_stateStartMs = millis();
        s_lastTxMs = 0U;
        SetLinkStatus(false, -100, -100);
        ...
        if ((ev.sessionId == s_sessionId) && (p->sessionId == s_sessionId) && (memcmp(p->devProof, s_devProof, 16U) == 0)) {
            (void)Line("@LINK AUTH_OK");
            // RP switches to encrypted peer only after this ACK.
            // Keep plaintext peer for a short delay so async esp_now_send() can transmit ACK.
            (void)SendAckTo(ev.mac, ev.seq, 0U);
            s_state = PEC_SECURE_WAIT;
            s_secureInstallAtMs = millis() + PEC_SECURE_DELAY_MS;
        } else {
            ResetToDiscovery("@LINK AUTH_BAD");
        }
    } else if ((ev.type == PEC_EVT_PONG) && SecureEventOk(ev)) {
        const auto* p = reinterpret_cast<const pen_pong_payload_t*>(ev.payload);
        s_lastPongMs = millis();
        SetLinkStatus(true, p->rssiAtPeer, ev.rssi);
    } else if (ev.type == PEC_EVT_VAR_I) {
        HandleVarI(ev);
    } else if (ev.type == PEC_EVT_VAR_F) {
        HandleVarF(ev);
    } else if (ev.type == PEC_EVT_ACK) {
        HandleAck(ev);
    } else if (ev.type == PEC_EVT_NACK) {
        HandleNack(ev);
    }
}
...
static void DrainEvents(void) {
    for (size_t i = 0; i < PEC_EVENT_CAP; ++i) {
        ...
        if (s_events[i].used) {
            used = true;
            ev.type = s_events[i].type;
            ev.msgType = s_events[i].msgType;
            ev.seq = s_events[i].seq;
            ev.sessionId = s_events[i].sessionId;
            memcpy(ev.mac, (const void*)s_events[i].mac, 6U);
            ev.rssi = s_events[i].rssi;
            memcpy(ev.payload, (const void*)s_events[i].payload, sizeof(ev.payload));
...
static void ReportCounters(void) {
    ...
    uint32_t badFrames;
    portENTER_CRITICAL(&s_mux);
    drops = s_eventDropCount;
    badFrames = s_badFrameCount;
    portEXIT_CRITICAL(&s_mux);

    if (drops != s_reportedEventDropCount) {
        EmitError(PEC_HW_ERR_EVENT_DROP, (int32_t)(drops - s_reportedEventDropCount));
        s_reportedEventDropCount = drops;
    }
    if (badFrames != s_reportedBadFrameCount) {
        EmitError(PEC_HW_ERR_BAD_FRAME, (int32_t)(badFrames - s_reportedBadFrameCount));
...
static void Task(void*) {
    ...
    s_rcId = pen_make_id_from_efuse();
    (void)Line("@LINK READY");
    ...
    for (;;) {
        ...
        if ((s_state == PEC_DISCOVERY) && ((now - s_lastTxMs) >= PEC_DISCOVERY_MS)) {
            s_lastTxMs = now;
            (void)SendDiscovery();
        } else if (s_state == PEC_CONNECTING) {
            if ((now - s_stateStartMs) > PEC_AUTH_TIMEOUT_MS) ResetToDiscovery("@LINK CONN_TO");
            else if ((now - s_lastTxMs) >= PEC_RETRY_MS) { s_lastTxMs = now; (void)SendConnect(); }
        } else if (s_state == PEC_AUTHING) {
            if ((now - s_stateStartMs) > PEC_AUTH_TIMEOUT_MS) ResetToDiscovery("@LINK AUTH_TO");
            else if ((now - s_lastTxMs) >= PEC_RETRY_MS) { s_lastTxMs = now; (void)SendAuth(); }
        } else if (s_state == PEC_SECURE_WAIT) {
            if ((s_secureInstallAtMs != 0U) && ((int32_t)(now - s_secureInstallAtMs) >= 0)) {
                if (!InstallEncryptedPeer()) ResetToDiscovery("@LINK SEC_BAD");
            }
        } else if (s_state == PEC_SECURE) {
            if ((now - s_lastPongMs) > PEC_LINK_TIMEOUT_MS) ResetToDiscovery("@LINK LOST");
            else if ((now - s_lastTxMs) >= PEC_PING_MS) { s_lastTxMs = now; (void)SendPing(); }
        }
...
static bool LooksFloat(const char* text) {
    ...
    return (strchr(text, '.') != nullptr) || (strchr(text, 'e') != nullptr) || (strchr(text, 'E') != nullptr);
}
...
bool pec_begin(pec_line_fn_t lineFn, BaseType_t coreId, UBaseType_t priority, uint32_t stackSize) {
    ...
    s_localRssi = -100;
    s_battery = 0U;
    s_errorCode = PEC_HW_ERR_NONE;
    s_errorDetail = 0;
    s_eventDropCount = 0U;
    s_badFrameCount = 0U;
    s_reportedEventDropCount = 0U;
...
bool pec_is_connected(void) {
    return s_state == PEC_SECURE;
}

void pec_get_status(pec_status_t* outStatus) {
    if (outStatus == nullptr) return;
    portENTER_CRITICAL(&s_mux);
    outStatus->connected = s_connected;
    outStatus->battery = s_battery;
    outStatus->peerRssi = s_peerRssi;
    outStatus->localRssi = s_localRssi;
    outStatus->errorCode = s_errorCode;
    outStatus->errorDetail = s_errorDetail;
    portEXIT_CRITICAL(&s_mux);
}
...
bool pec_send_get_var(uint32_t varId) {
    ...
    if (!ok) EmitError(PEC_HW_ERR_SEND, MSG_GET_VAR);
    return ok;
}
