...
static bool MsgRetry(uint8_t t) {
  return (t & PEN_MSG_FLAG_RETRY) != 0U;
}
static uint8_t MsgMake(uint8_t t, bool retry) {
  return (uint8_t)((t & PEN_MSG_TYPE_MASK) | (retry ? PEN_MSG_FLAG_RETRY : 0U));
}
static bool MsgState(uint8_t t) {
  t = MsgBase(t);
  return (t == MSG_STATE_I_VAR) || (t == MSG_STATE_F_VAR);
}
static bool MsgEvent(uint8_t t) {
  t = MsgBase(t);
  return (t == MSG_EVENT_I_VAR) || (t == MSG_EVENT_F_VAR);
}
static bool MsgAcked(uint8_t t) {
  return MsgState(t) || MsgEvent(t);
}
static bool MsgSecure(uint8_t t) {
  t = MsgBase(t);
  return (t == MSG_STREAM_I_VAR) || (t == MSG_STREAM_F_VAR) || (t == MSG_STATE_I_VAR) || (t == MSG_STATE_F_VAR) || (t == MSG_EVENT_I_VAR) || (t == MSG_EVENT_F_VAR) || (t == MSG_ACK) || (t == MSG_NACK) || (t == MSG_GET_VAR);
}

static uint32_t MakeId(void) {
  return (uint32_t)(ESP.getEfuseMac() & 0xFFFFFFFFULL);
}
static bool GetStaMac(uint8_t out[6]) {
  return (out != nullptr) && (esp_wifi_get_mac(WIFI_IF_STA, out) == ESP_OK);
}
static bool MacSet(const uint8_t mac[6]) {
  static const uint8_t zero[6] = {};
  return (mac != nullptr) && (memcmp(mac, zero, 6U) != 0);
}
static const uint8_t* Payload(const frame_evt_t& ev) {
...
static uint16_t Crc16(const void* data, size_t len) {
  return (data == nullptr) ? 0U : esp_crc16_le(PEN_CRC_INIT, reinterpret_cast<const uint8_t*>(data), (uint32_t)len);
}
...
static void ShaLabel(const char* label, const uint8_t key[32], uint8_t out[32]) {
  ...
  ShaStart(&ctx);
  ShaUpdate(&ctx, key, 32U);
  ShaUpdate(&ctx, label, strlen(label));
  ShaFinish(&ctx, out);
}
static void DeriveSession(uint32_t rcId, uint32_t devId, uint32_t sessionId, const uint8_t rcMac[6], const uint8_t devMac[6], const uint8_t rcNonce[16], const uint8_t devNonce[16], uint8_t key[32], uint8_t pmk[16], uint8_t lmk[16], uint8_t rcProof[16], uint8_t devProof[16]) {
  ...
  ShaUpdate(&ctx, rcMac, 6U);
  ShaUpdate(&ctx, devMac, 6U);
  ShaUpdate(&ctx, rcNonce, 16U);
  ShaUpdate(&ctx, devNonce, 16U);
  ShaFinish(&ctx, key);
  ShaLabel("PMK", key, tmp);
  memcpy(pmk, tmp, 16U);
  ShaLabel("LMK", key, tmp);
  memcpy(lmk, tmp, 16U);
  ShaLabel("RC", key, tmp);
  memcpy(rcProof, tmp, 16U);
  ShaLabel("DEV", key, tmp);
  memcpy(devProof, tmp, 16U);
}
...
static bool IsSecurePeer(const frame_evt_t& ev) {
  return (s_state == ST_SECURE) && s_peerValid && (ev.sessionId == s_sessionId) && (memcmp(ev.mac, s_peerMac, 6U) == 0);
}
...
static void SetStreamIValue(uint32_t varId, int32_t value, uint16_t ttlMs, bool touchTxTime) {
  ...
  for (size_t i = 0; i < STREAM_CAP; ++i) {
    if (s_streams[i].used && !s_streams[i].isFloat && (s_streams[i].varId == varId)) {
      slot = i;
      break;
    }
...
static void CacheStreamI(uint32_t varId, int32_t value, uint16_t ttlMs) {
  SetStreamIValue(varId, value, ttlMs, true);
}
...
static bool SendStreamIRaw(uint32_t varId, int32_t value, bool retryFlag = false) {
  pen_var_i_payload_t p = {};
  p.varId = varId;
  p.value = value;
  const bool ok = SendFrame(s_peerMac, MsgMake(MSG_STREAM_I_VAR, retryFlag), s_sessionId, s_seq++, p);
  if (ok) s_lastTxMs = millis();
  else EmitError(PEN_HW_ERR_SEND, MSG_STREAM_I_VAR);
  return ok;
}
static bool SendStreamFRaw(uint32_t varId, float value, bool retryFlag = false) {
  pen_var_f_payload_t p = {};
  p.varId = varId;
  p.value = value;
  const bool ok = SendFrame(s_peerMac, MsgMake(MSG_STREAM_F_VAR, retryFlag), s_sessionId, s_seq++, p);
  if (ok) s_lastTxMs = millis();
...
static bool SendHeartbeat(uint32_t now) {
  ...
  for (size_t i = 0; i < STREAM_CAP; ++i) {
    ...
    if ((s_streams[i].ttlMs != 0U) && (age >= s_streams[i].ttlMs) && ((expired < 0) || (age > expiredAge))) {
      expired = (int)i;
      expiredAge = age;
    }
  }
...
static void ProcessPending(uint32_t now) {
  for (size_t i = 0; i < PENDING_CAP; ++i) {
    pending_tx_t item = {};
    bool resend = false, timeout = false;
    portENTER_CRITICAL(&s_mux);
    if (s_pending[i].used && ((now - s_pending[i].lastTxMs) >= ACK_TIMEOUT_MS)) {
      if (s_pending[i].retryCount >= MAX_RETRY) {
        item = s_pending[i];
        memset(&s_pending[i], 0, sizeof(s_pending[i]));
        timeout = true;
      } else {
...
static void RecentRemember(uint8_t msgType, uint16_t seq, uint32_t varId) {
  for (size_t i = RECENT_CAP - 1U; i > 0U; --i) s_recent[i] = s_recent[i - 1U];
...
static bool SendVarI(uint8_t msgType, uint32_t varId, int32_t value, uint16_t ttlMs) {
  ...
  if (ok) {
    ...
    if (MsgAcked(msgType)) (void)PendingRemove(seq);
...
static bool EmitVarI(uint8_t msgType, uint32_t varId, int32_t value, bool retry) {
  pen_rx_event_t out = {};
  out.type = PEN_RX_VAR_I;
  out.msgType = MsgBase(msgType);
  out.data.varI.varId = varId;
  out.data.varI.value = value;
  out.data.varI.retry = retry;
  return QueueApp(out);
}
static bool EmitVarF(uint8_t msgType, uint32_t varId, float value, bool retry) {
  pen_rx_event_t out = {};
  out.type = PEN_RX_VAR_F;
  out.msgType = MsgBase(msgType);
  out.data.varF.varId = varId;
  out.data.varF.value = value;
  out.data.varF.retry = retry;
  return QueueApp(out);
}
...
static void HandleVarI(const frame_evt_t& ev) {
  ...
  const bool reliable = MsgAcked(ev.msgType);
  const bool duplicate = reliable && MsgRetry(ev.msgType) && RecentSeen(ev.msgType, ev.seq, p->varId);
  const bool retry = reliable ? duplicate : MsgRetry(ev.msgType);

  if (duplicate) {
    (void)QueueRxVarI(p, ev, true);
    (void)SendAckTo(ev.mac, ev.seq, p->varId);
    return;
  }

  const bool handled = QueueRxVarI(p, ev, retry);
  if (reliable) {
    if (handled) {
...
static void HandleVarF(const frame_evt_t& ev) {
  ...
  if (duplicate) {
    (void)QueueRxVarF(p, ev, true);
    (void)SendAckTo(ev.mac, ev.seq, p->varId);
    return;
  }

  const bool handled = QueueRxVarF(p, ev, retry);
  if (reliable) {
    if (handled) {
      RecentRemember(ev.msgType, ev.seq, p->varId);
      (void)SendAckTo(ev.mac, ev.seq, p->varId);
...
static void HandleAck(const frame_evt_t& ev) {
...
static void HandleNack(const frame_evt_t& ev) {
...
static void ReportCounters(void) {
  ...
  ad = s_appDropCount;
  portEXIT_CRITICAL(&s_mux);
  if (fd != s_reportedFrameDropCount) {
    EmitError(PEN_HW_ERR_EVENT_DROP, (int32_t)(fd - s_reportedFrameDropCount));
    s_reportedFrameDropCount = fd;
  }
  if (bf != s_reportedBadFrameCount) {
    EmitError(PEN_HW_ERR_BAD_FRAME, (int32_t)(bf - s_reportedBadFrameCount));
    s_reportedBadFrameCount = bf;
  }
  if (ad != s_reportedAppDropCount) {
    EmitError(PEN_HW_ERR_RX_DROP, (int32_t)(ad - s_reportedAppDropCount));
...
bool pen_send_event(uint32_t varId, int32_t value) {
  return SendVarI(MSG_EVENT_I_VAR, varId, value, 0U);
}
