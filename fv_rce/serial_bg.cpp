...
static void serial_bg_tx_task(void* arg) {
  ...
  for (;;) {
    if (xQueueReceive(s_txQueue, &msg, portMAX_DELAY) != pdTRUE) continue;
    if (!s_connected) continue;
    if ((s_serialMutex != NULL) && (xSemaphoreTake(s_serialMutex, pdMS_TO_TICKS(20U)) == pdTRUE)) {
      if (s_connected) {
...
bool serial_bg_begin(uint32_t baud, bool connected, BaseType_t coreId, UBaseType_t priority, uint32_t stackSize) {
  if (s_started) return true;
  s_rxQueue = xQueueCreate(SERIAL_BG_QUEUE_DEPTH, sizeof(serial_bg_msg_t));
  if (s_rxQueue == NULL) return false;
  ...
  if (xTaskCreatePinnedToCore(serial_bg_rx_task, "SerialBgRx", s_stackSize, NULL, s_priority, &s_rxTaskHandle, s_coreId) != pdPASS) {
  ...
  if (xTaskCreatePinnedToCore(serial_bg_tx_task, "SerialBgTx", s_stackSize, NULL, s_priority, &s_txTaskHandle, s_coreId) != pdPASS) {
...
void serial_bg_set_connected(bool connected) {
  ...
  if (xSemaphoreTake(s_serialMutex, pdMS_TO_TICKS(20U)) != pdTRUE) return;
  if (connected) {
...
bool serial_bg_send_line(const char* text) {
  ...
  if ((!s_started) || (!s_connected) || (s_txQueue == NULL) || (text == NULL)) return false;
  len = strnlen(text, SERIAL_BG_LINE_CAP);
  if ((len == 0U) || (len >= SERIAL_BG_LINE_CAP)) return false;
...
bool serial_bg_receive_line(char* outLine, size_t outCap) {
  ...
  if ((outLine == NULL) || (outCap == 0U) || (s_rxQueue == NULL)) return false;
  if (xQueueReceive(s_rxQueue, &msg, 0U) != pdTRUE) return false;
